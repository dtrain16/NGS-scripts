...
sample_info <- data.frame(
  sample    = c("dxo1_1", "dxo1_2", "WT_1", "WT_2"),
  condition = rep(c("dxo1", "WT"), each = 2),
  replicate = rep(1:2,2),
  bam       = sapply(
    c("dxo1_rep1Aligned.sortedByCoord.out.bam", 
      "dxo1_rep2Aligned.sortedByCoord.out.bam",
  ...
  coverage  = file.path(
    working_directory,
    paste0(c("dxo1_1", "dxo1_2", "WT_1", "WT_2"), ".rds")
  )
)
...
write_tsv(out2, "dxo1_DERs.tsv", col_names=F)

out3 <- select(out2, seqnames, start, end, featureId, log2FoldChange, strand)
write_tsv(out3, "dxo1_DERs.bed", col_names=F)
