...
ggplot(data = dge) + aes(x=logFC, y=-log10(FDR), group=contrast, colour=DGE) +
  ...
  scale_colour_manual(values = c('Down-regulated'="royalblue","Non-DGE"="lightgrey","Up-regulated"="khaki1"))
...
subset(a_venn, EL == 1) %>% select(geneID)%>% write_csv("Smith_EL_low_halflife.csv")
...
half_ratio <- out_all %>% 
	...
	mutate(logfc = dge2$logFC[match(geneID,dge2$ID)]) %>%
	mutate(logcpm = dge2$logCPM[match(geneID,dge2$ID)]) %>%
	mutate(fold_change = 2^logfc) %>%
...
p1 <- ggplot(data=half_ratio) + aes(x=t_ratio_HL, y=fold_change) + 
	theme_niwot() +
...
p2 <- ggplot(data=half_ratio) + aes(x=logcpm, y=t_ratio_HL) + 
	theme_niwot() +
        geom_point() +
...
summary(lm(formula = fold_change ~ t_ratio_HL, data = half_ratio))
cor(half_ratio$fold_change, half_ratio$t_ratio_HL)
  subset(rrgd_loci == "RRGD")

##################
## mRNA half-life vs translation (polysome-RNA abundance and RPL)
#################

polyR <- dir("polysome_rna/") %>% tibble() %>% 
	...
	mutate(t_ratio_REC = half_ratio$t_ratio_HL[match(ID, half_ratio$geneID)]) %>%
	na.omit()  %>%
...
p1 <- ggplot(data=polyR) + aes(x=log2(t_ratio_HL), y=log2(fc_30)) +
	theme_niwot() +
        geom_point(aes(colour=loci, alpha=loci)) +
        geom_smooth(method='lm', formula = y~x) +
        scale_y_continuous(name = "log2FC RPL", breaks=c(-5,0,5)) +
	geom_hline(yintercept = c(log2(1.5),-log2(1.5)), type = 'dashed') +
        scale_x_continuous(name = "log2FC half-life \n (HL : US)", breaks=c(-5,0,5)) +
	geom_vline(xintercept = c(log2(1.5),-log2(1.5)), type = 'dashed') +
        scale_colour_manual(values = c("black", "salmon")) +
        scale_alpha_manual(values = c(0.4,1.4)) +
	theme(legend.position="none") +
	coord_cartesian(ylim=c(-6,6), xlim=c(-6,6))

p2 <- ggplot(data=polyR) + aes(x=t_ratio_HL, y=fc_60) +
        theme_niwot() +
        geom_point(aes(colour=loci, alpha=loci)) +
        geom_smooth(method='lm', formula = y~x) +
        scale_y_continuous(name = "Fold-change RPL", trans = "log10", breaks=c(0.1,1,10)) +
        scale_x_continuous(name = "mRNA half-life ratio \n (HL : US)", trans = "log10") +
        scale_colour_manual(values = c("black", "salmon")) +
        scale_alpha_manual(values = c(0.4,1.4)) +
	theme(legend.position="none") +
	coord_cartesian(ylim=c(0.05,40))

p3 <- ggplot(data=polyR) + aes(x=t_ratio_REC, y=fc_90) +
        theme_niwot() +
        geom_point(aes(colour=loci, alpha=loci)) +
        geom_smooth(method='lm', formula = y~x) +
        scale_y_continuous(name = "Fold-change RPL", trans = "log10", breaks=c(0.1,1,10)) +
...
a <- subset(polyR, time == "t0")
cor(x=a$halflife, y=a$poly, method="pearson")
lm(formula = poly ~ halflife, data = a)
cor(x=a$halflife, y=a$rpl, method="pearson")
lm(formula = rpl ~ halflife, data = a)

a <- subset(polyR, Condition == "t30")
cor(x=a$halflife, y=a$poly, method="pearson")
lm(formula = poly ~ halflife, data = a)
cor(x=a$halflife, y=a$rpl, method="pearson")
lm(formula = rpl ~ halflife, data = a)

a <- subset(polyR, Condition == "t60")
cor(x=a$halflife, y=a$poly, method="pearson")
lm(formula = poly ~ halflife, data = a)
cor(x=a$halflife, y=a$rpl, method="pearson")
lm(formula = rpl ~ halflife, data = a)

a <- subset(polyR, Condition == "t90")
cor(x=a$halflife, y=a$poly, method="pearson")
lm(formula = poly ~ halflife, data = a)
cor(x=a$halflife, y=a$rpl, method="pearson")
lm(formula = rpl ~ halflife, data = a)
