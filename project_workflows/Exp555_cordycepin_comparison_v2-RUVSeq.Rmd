...
tmp1 <- subset(tmp, Condition == "NL")
tmp2 <- subset(tmp, Condition == "EL")
tmp3 <- subset(tmp, Condition == "Rec")

possibleError <- tryCatch(mdl1 <- lmer(log_e ~ Timepoint + (1| Experiment), data=tmp1), 
	error = function(e) e)

if ( !inherits(possibleError, "error")) {
	mdl1 <- lmer(log_e ~ Timepoint + (1|Experiment), data=tmp1)

	rsq <- rsquared(mdl1)$Conditional
	k <- -coefficients(mdl1)$Experiment$Timepoint[1]	
	bic <- BIC(mdl1)
	a1 <- data.frame(geneID=paste(i), bic, rsq, k, Condition=tmp1$Condition[1])	
	...
	mdl2 <- lmer(log_e ~ Timepoint + (1| Experiment), data=subset(tmp, Condition == "EL"))
	
	rsq <- rsquared(mdl2)$Conditional
	k <- -coefficients(mdl2)$Experiment$Timepoint[1]
	bic <- BIC(mdl2)
	a2 <- data.frame(geneID=paste(i), bic, rsq, k, Condition="High-light")	

	mdl3 <- lmer(log_e ~ Timepoint + (1| Experiment), data=subset(tmp, Condition == "Rec"))

	rsq <- rsquared(mdl3)$Conditional
	k <- -coefficients(mdl3)$Experiment$Timepoint[1]
	bic <- BIC(mdl3)
	a3 <- data.frame(geneID=paste(i), bic, rsq, k, Condition="Recovery")

tmp <- rbind(a1,a2,a3)
tmp$half_life <- log(2)/tmp$k
