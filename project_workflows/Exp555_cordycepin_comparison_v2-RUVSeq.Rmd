...
tmp <- subset(normData, ID == i)
	...
	if ( !inherits(possibleError, "error")) {
		...
		half_life <- log(2)/k
		a1 <- data.frame(geneID=i, loglik, rsq, k, half_life, Condition=c)	
...
p1 <- ggplot(out) + aes(x=loglik, colour=Condition, fill=Condition) + 
	geom_histogram(alpha=0.3, position="identity") +
	theme_niwot() +
	theme(legend.position = 'none') +
	scale_x_continuous(name = "Log-likelihood")
