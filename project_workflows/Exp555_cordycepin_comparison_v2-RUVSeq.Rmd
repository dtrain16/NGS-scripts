...
###########################
#### evaluate RNA pool change using DGE in cordycepin samples
###########################
...
for(i in colnames(my.contrasts)){
  ...
  input <- rbind(input,tt)
}

input <- mutate(input, DGE=ifelse(logFC > 0 & FDR < 0.05, yes = "Up-regulated", no=
                      ifelse(logFC < 0 & FDR < 0.05, yes='Down-regulated', no="Non-DGE")))

pdf("DGE_testing.pdf")
ggplot(data = input) + aes(x=logFC, y=-log10(FDR), group=contrast, colour=DGE) +
...
for( i in unique(normData$ID)){
	...
	tmp <- subset(normData, ID == i & Condition == "NL")
	tmp_out <- NULL
	...
	if ( !inherits(possibleError, "error")) {
		...
		rsq <- rsquared(fit)$Conditional
                ...
                halflife <- log(2)/k
                a1 <- data.frame(geneID=i, rsq, aic, k, halflife, Condition=tmp$Condition[1])
...
## filter genes
out_NL <- subset(out1, rsq >= 0.5 & halflife > 0 & halflife < 1440)

p2 <- ggplot(out_NL) + aes(x=rsq) +
...
p3 <- ggplot(out_NL) + aes(x=k) +
        geom_histogram(position="identity", col='black') +
        theme_niwot() +
        theme(legend.position = 'none') +
        scale_x_continuous(name = "k-decay", breaks=c(0, 0.15))

p1 <- ggplot(out_NL) + aes(x=aic) +
        geom_histogram(position="identity", col='black') +
        theme_niwot() +
        theme(legend.position = 'none') +
        scale_x_continuous(name = "AIC")

p5 <- ggplot(out_NL) + aes(x=rsq, y=halflife) +
        geom_point() +
        theme_niwot() +
	coord_cartesian(xlim=c(0,1)) +
        scale_x_continuous(name = "R-sq", breaks=c(0, 0.5, 1))+
...
p6 <- ggplot(out_NL) + aes(x=aic, y=halflife) +
        geom_point() +
        theme_niwot() +
        scale_x_continuous(name = "AIC")+
        scale_y_continuous(name = "Half-life (min)")
...
o1 <- subset(out, Condition == "NL")
o2 <- subset(out, Condition == "EL")
o3 <- subset(out, Condition == "Rec")

row_grps <- dplyr::select(normData, ID) %>% unique %>% tibble %>%
	mutate(rsq1 = o1$rsq[match(ID, o1$geneID)], rsq2= o2$rsq[match(ID, o2$geneID)],  rsq3= o3$rsq[match(ID, o3$geneID)]) %>%
	column_to_rownames("ID")

pheatmap(as.matrix(heatmap_input),
        annotation_col = my_grps,
	annotation_row = row_grps,
        cluster_cols=F,
...
w1 <- rbind(b1,b2,b3,b4,b5) %>%
	...
	mutate(Condition = factor(Condition, levels = c("NL","Narsai", "Sorenson", "Szabo", "Chantarachot"))) %>%
	mutate(geneID = as.character(geneID)) %>%
	mutate(log_halflife = log2(halflife + 0.01))

test2 <- dabest(.data = w1, x = Condition, y = log_halflife, 
	idx = c("NL", "Sorenson", "Szabo"),
	#idx = c("NL","Narsai", "Sorenson", "Szabo", "Chantarachot"), 
	paired=FALSE)

pdf("dabest_v1.pdf")
plot(cliffs_delta(test2, reps=10000), rawplot.ylabel = "log2[half-life + 0.01] (min)")
dev.off()
