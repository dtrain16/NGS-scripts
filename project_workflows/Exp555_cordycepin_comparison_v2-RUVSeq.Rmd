...
plot(test3, rawplot.ylabel = "log2 half-life + 0.01 (min)")
...
polyR <- dir("polysome_rna/") %>% tibble() %>% 
	...
	subset(time == 't0' | time == 't30' | time == 't90') %>%
	spread(type, avg_rpkm) %>%
	mutate(rpl = poly/total) %>%
	mutate(Condition = ifelse(time == "t30", "EL", ifelse(time == "t0", "NL", "Rec"))) %>%
	left_join(out_all, by=c("ID" = "geneID", "Condition" = "Condition")) %>%
	subset(is.na(halflife) == F) %>%
	mutate(Condition = factor(Condition, levels = c("NL","EL","Rec"))) %>%
	na.omit()

pdf("poly_vs_halflife.pdf")
p1 <- ggplot(data=polyR) + aes(x=halflife, y=poly) +
	geom_point(aes(colour=Condition)) +
	theme_niwot() +
	facet_wrap(~Condition, nrow=1) + 
	geom_smooth(method="lm") +
	scale_y_continuous(name="Mean RPKM", trans='log10') +
        scale_x_continuous(name="Half-life (min)")

p2 <- ggplot(data=polyR) + aes(x=halflife, y=rpl) +
        geom_point(aes(colour=Condition)) +
        theme_niwot() +
        facet_wrap(~Condition, nrow=1) +
	geom_smooth(method="lm") +
        scale_y_continuous(name="Mean RPL", trans='log10') +
        scale_x_continuous(name="Half-life (min)")
...
a <- subset(polyR, Condition == "EL")
cor(x=a$halflife, y=a$poly, method="pearson")
lm(formula = poly ~ halflife, data = a)
cor(x=a$halflife, y=a$rpl, method="pearson")
lm(formula = rpl ~ halflife, data = a)

a <- subset(polyR, Condition == "Rec")
cor(x=a$halflife, y=a$poly, method="pearson")
lm(formula = poly ~ halflife, data = a)
cor(x=a$halflife, y=a$rpl, method="pearson")
lm(formula = rpl ~ halflife, data = a)
