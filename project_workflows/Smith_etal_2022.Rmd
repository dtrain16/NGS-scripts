...
## write out HL 30-60 genes that have increased RPL
test_rpl <- subset(polyR, cat_60 == "g1")

write_csv(test_rpl, "HL_60_RPLup.csv")

## write out HL 30-60 genes that have increased RPL but no stability change
test_rpl <- subset(polyR, cat_60 == "g1") %>%
	select(ID, logfc_60, t_ratio_HL, log_half_HL, cat_60) %>%
	subset(log_half_HL <= log(1.5) & log_half_HL >= -log(1.5))

write_csv(test_rpl, "HL_60_prefRPLup.csv")
