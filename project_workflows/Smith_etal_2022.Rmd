...
#pcomp1 <- prcomp(t(as.matrix(gene_tpm[keep,])), center = T, scale. = T)
#pcomp2 <- prcomp(t(cpm(normCounts(set1))), center=T, scale. = T)
...
#        pointsize=1.5, invisible="quali", addEllipses=F, ellipse.type = "confidence")

#my_grps1 <- pData(set1) %>% subset(Treatment == "Cordycepin") %>%
#        mutate(group = paste(Condition, Timepoint, sep='_')) %>%
#        dplyr::select(Condition, Timepoint, Experiment) %>%
#        mutate(Experiment = as.factor(Experiment))

#cor_matrix <- cor(test1, method="pearson")
#pheatmap(cor_matrix, annotation_col = my_grps1, scale="none", cluster_cols=T,
#        cluster_rows=T, show_colnames=F, show_rownames=F, fontsize=6)
...
#ggplot(data = dge) + aes(x=logFC, y=-liog10(FDR), group=contrast, colour=DGE) +
#  geom_jitter(size=0.8) + theme_niwot() + facet_wrap(~contrast) +
...
sample_ids <- test2$locus_identifier[96:99]
...
a_venn <- subset(group == "EL") %>% select(a, geneID, group, test) %>% spread(group, test)
