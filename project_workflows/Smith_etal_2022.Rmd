...
half_ratio_rrgd <- subset(half_ratio, loci == "RRGD") %>%
	dplyr::select(geneID, t_ratio_HL, t_ratio_Rec, fold_change_HL, fold_change_Rec) %>% gather(type, value, -geneID) %>%
	mutate(condition = sapply(strsplit(type, "_"), function(l) l[3])) %>%
	mutate(type = sapply(strsplit(type, "_"), function(l) paste(l[1],l[2],sep="_")))%>%
	spread(type, value)
...
p3 <- ggplot(data=half_ratio_rrgd) + aes(x=t_ratio, y=fold_change) +
	facet_wrap(~condition) +
       	theme_niwot() +
        geom_point(col='salmon') +
        scale_y_continuous(name = "mRNA relative abundance", trans = "log10") +
       	scale_x_continuous(name = "mRNA half-life ratio", trans = "log10") +
        geom_vline(xintercept=1, col='dodgerblue', linetype='dashed')+
	geom_hline(yintercept=1, col='dodgerblue', linetype='dashed')
...
summary(lm(formula = fold_change_Rec ~ t_ratio_REC, data = half_ratio_rrgd))
cor(half_ratio_rrgd$fold_change_Rec, half_ratio_rrgd$t_ratio_REC)
...
polyR <- dir("polysome_rna/") %>% tibble() %>% 
	...
	na.omit()  %>%
        ...
        mutate(loci = ifelse(is.na(loci) == T, yes="non", no="RRGD")) %>%
        mutate(loci = factor(loci, levels = c("non","RRGD"))) %>%
	mutate(cat_30 = ifelse(logfc_30 >= log2(1.5), 'g1', 
		ifelse(logfc_30 <= -log2(1.5), 'g2', 'g3'))) %>% 
	mutate(cat_30 = ifelse(cat_30 != 'g3', cat_30, 
		ifelse(log_half_HL>=log2(1.5),'g4', 
			ifelse(log_half_HL <= -log2(1.5),'g5','g6')))) %>%
	mutate(cat_60 = ifelse(logfc_60 >= log2(1.5), 'g1',                                                       ifelse(logfc_60 <= -log2(1.5), 'g2', 'g3'))) %>%
        mutate(cat_60 = ifelse(cat_60 != 'g3', cat_60,
                ifelse(log_half_HL>=log2(1.5),'g4',                                                                       ifelse(log_half_HL <= -log2(1.5),'g5','g6')))) %>%
	mutate(cat_90 = ifelse(logfc_90 >= log2(1.5), 'g1',                                                       ifelse(logfc_90 <= -log2(1.5), 'g2', 'g3'))) %>%
        mutate(cat_90 = ifelse(cat_90 != 'g3', cat_90,
                ifelse(log_half_REC>=log2(1.5),'g4',                                                                       ifelse(log_half_REC <= -log2(1.5),'g5','g6'))))
...
polyR_rpkm <- dir("polysome_rna/") %>% tibble() %>%
        mutate(file_contents = map(., ~read_csv(file.path("polysome_rna", .)))) %>%
        unnest(cols=c(file_contents)) %>%
        gather(sample, rpkm, -., -ID, -Length, na.rm = T) %>%
        mutate(type = sapply(strsplit(sample, "_"), function(l) l[3])) %>%
