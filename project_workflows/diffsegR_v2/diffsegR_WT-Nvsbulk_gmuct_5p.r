...
sample_info <- data.frame(
	sample    = c("WT.N_1", "WT.N_2", "WT.N_3", "BDG_1", "BDG_2", "HMC_1", "HMC_2"),
	condition = c(rep("WT.N", each = 3),rep("BDG", each=2),rep("HMC",each=2)),
	replicate = c(1:3,1:2,1:2),
	bam       = sapply(
		c("S5-3N_Aligned.sortedByCoord.out.bam", 
		"S7-4N_Aligned.sortedByCoord.out.bam",
		"S11-10N_Aligned.sortedByCoord.out.bam",
		"WT-BDG_rep1_Aligned.sortedByCoord.out.bam",
		"WT-BDG_rep2_Aligned.sortedByCoord.out.bam",
		"WT-HMC_rep1_Aligned.sortedByCoord.out.bam",
		"WT-HMC_rep2_Aligned.sortedByCoord.out.bam"),
	function(bam) file.path(working_directory, bam)),
	isPairedEnd = c(rep(TRUE, 3),rep(FALSE,4)),
	strandSpecific = rep(0, 7)
)
