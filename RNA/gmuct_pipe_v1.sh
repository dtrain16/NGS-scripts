...
# Read alignment for GMUCT libraries with stringent STAR parameters
...
echo "USAGE: gmuct_pipe_v1.sh <SE/PE> <fastq R1> <R2> </path/to/index> <fileID>"
echo "EXAMPLE: gmuct_pipe_v1.sh SE sample.fastq /home/dganguly/ref_seqs/STAR/TAIR10/GenomeDir sample_rep1"
exit 1
fi
...
echo "USAGE: gmuct_pipe_v1.sh <SE> <R1> </path/to/index> <fileID>"
echo "EXAMPLE: gmuct_pipe_v1.sh SE sample.fastq /home/dganguly/ref_seqs/STAR/TAIR10/GenomeDir sample_rep1"
exit 1
fi
...
mkdir ${fileID}_gmuct_${dow}
mv $fq ${fileID}_gmuct_${dow}
cd ${fileID}_gmuct_${dow}
...
STAR --runThreadN 8 --alignIntronMax 3000 --outFilterMismatchNmax 0 --outFilterScoreMinOverLread 0.75 --outFilterMatchNminOverLread 0.75 --outFilterMultimapNmax 1 --genomeDir $index --readFilesCommand gunzip -c --readFilesIn $input --outFileNamePrefix "${fileID}_" --outSAMtype BAM SortedByCoordinate --limitBAMsortRAM 8000000000 | tee -a  ../${fileID}_logs_${dow}.log
...
echo "USAGE: gmuct_pipe_v1.sh <PE> <R1> <R2> </path/to/index> <fileID>"
echo "EXAMPLE: gmuct_pipe_v1.sh PE sample_R1.fq sample_R2.fq /home/dganguly/STAR/TAIR10/GenomeDir sample_rep1"
exit 1
fi
...
# make sample work directory
mkdir ${fileID}_gmuct_${dow}
mv $fq1 ${fileID}_gmuct_${dow}
mv $fq2 ${fileID}_gmuct_${dow}
cd ${fileID}_gmuct_${dow}
...
STAR --runThreadN 8 --alignIntronMax 3000 --outFilterMismatchNmax 0 --outFilterScoreMinOverLread 0.75 --outFilterMatchNminOverLread 0.75 --outFilterMultimapNmax 1 --genomeDir $index --readFilesCommand gunzip -c --readFilesIn $input1 $input2 --outFileNamePrefix "${fileID}_" --outSAMtype BAM SortedByCoordinate --limitBAMsortRAM 8000000000 | tee -a  ../${fileID}_logs_${dow}.log
