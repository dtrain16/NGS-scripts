...
if [[ "$str"  == "unstranded" ]] ; then 
	...
	awk -F$'\t' '$NF<50 && $NF>-50' ${smp%%.bam}_${out}.5p.bed > ${smp%%.bam}_${out}_50bp.5p.bed 
...
if [[ "$str"  == "forward" ]] ; then
	...
	closestBed -D "b" -a ${smp%%.bam}.minus.5p.bed -b $bedfile > ${smp%%.bam}_${out}.minus.5p.bed
        awk -F$'\t' '$NF<50 && $NF>-50' ${smp%%.bam}_${out}.minus.5p.bed > ${smp%%.bam}_${out}_50bp.minus.5p.bed
	...
	closestBed -D "b" -a ${smp%%.bam}.plus.5p.bed -b $bedfile > ${smp%%.bam}_${out}.plus.5p.bed
        awk -F$'\t' '$NF<50 && $NF>-50' ${smp%%.bam}_${out}.plus.5p.bed > ${smp%%.bam}_${out}_50bp.plus.5p.bed

	echo 'do maths'
	Rscript /home/dganguly/scripts/RNA/rel_expression_plots_stop.r ${smp%%.bam}_${out}_50bp.minus.5p.bed
