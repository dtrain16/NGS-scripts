...
# Retains exon number information
...
exon_3p <- subset(input, pos < 0 & pos > -51) %>%
	...
	mutate(gene=sapply(strsplit(V8, "\\."), function(l) l[1])) %>%
        mutate(exon=sapply(strsplit(V8, "\\."), function(l) l[3]))
