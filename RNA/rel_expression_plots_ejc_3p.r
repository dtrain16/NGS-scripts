...
exon_3p_sum <- subset(input, pos_3p < 0 & pos_3p > -51) %>%
	group_by(V8) %>%
...
sum_exon_3p <- group_by(exon_3p, pos_3p) %>% summarise(sum_norm_reads = sum(norm_reads)) %>% mutate(total_reads = sum(sum_norm_reads)) %>% mutate(rel_freq = sum_norm_reads/total_reads)
...
## output
write.table(sum_exon_3p, paste0(name,".3p.txt"), sep='\t', quote=F, row.names=F)
